...
def find_ζ_vol(x_min_RE,lower,bounds=(-1.1,-0.9),ζ_tol=1e-4,ξ_tol=1e-7,max_iter=100,print_option=False):
    """
    This function will use bisection method to find the ζ that minimizes/maximizes the risk premia.
    """
    # Initialize lower/upper bounds for ζ
    ...
    while True:
        ...
        if count == 0:
            # Calculate derivatives at the boundary points
            # Use left derivative at the lower bound
            objective_lower,_,_,_ = volatility(ζ_lower,x_min_RE=x_min_RE,lower=lower,ξ_tol=ξ_tol)
            objective_lower_plus,_,_,_ = volatility(ζ_lower+ζ_tol,x_min_RE=x_min_RE,lower=lower,ξ_tol=ξ_tol)
            derivative_lower = (objective_lower_plus-objective_lower)/ζ_tol
            # Use right derivative at the upper bound
            objective_upper,_,_,_ = volatility(ζ_upper,x_min_RE=x_min_RE,lower=lower,ξ_tol=ξ_tol)
            objective_upper_minus,_,_,_ = volatility(ζ_upper-ζ_tol,x_min_RE=x_min_RE,lower=lower,ξ_tol=ξ_tol)
            derivative_upper = (objective_upper-objective_upper_minus)/ζ_tol            
            if np.sign(derivative_lower) == np.sign(derivative_upper):
                raise Exception("It seems the optimal point is out of the initial interval! Please try a different one.") 
                break
        ...
        # Check sign of derivative at the mid point
            ...
            ζ_mid = (ζ_lower+ζ_upper)/2.
            objective_mid,_,_,_ = volatility(ζ_mid,x_min_RE=x_min_RE,lower=lower,ξ_tol=ξ_tol)
            objective_mid_minus,_,_,_ = volatility(ζ_mid-ζ_tol,x_min_RE=x_min_RE,lower=lower,ξ_tol=ξ_tol)
            objective_mid_plus,_,_,_  = volatility(ζ_mid+ζ_tol,x_min_RE=x_min_RE,lower=lower,ξ_tol=ξ_tol)
