...
def bound_ratio(find_ξ_args, g1, g2, ζ, lower=True, result_type=0):
    ...
    if lower:
        ...
        g = -(g1 - ζ*g2)
    # f, z0, z1, n_states, tol, max_iter
    f = find_ξ_args[0][0]
    z0 = find_ξ_args[0][1]
    z1 = find_ξ_args[0][2]
    n_states = find_ξ_args[0][3]
    solver_tol = find_ξ_args[0][4]
    solver_max_iter = find_ξ_args[0][5]
    solver_args = (f, g, z0, z1, n_states, solver_tol, solver_max_iter)
    result = solve(f, g, z0, z1, 100., n_states, solver_tol, solver_max_iter)
    if find_ξ_args[1] != 0:
