...
def entropy_moment_bounds():
    ...
    QDs_lower = np.array([result['QD'] for result in results_lower])
    QDs_upper = np.array([result['QD'] for result in results_upper])
    ...
    # Plots for QD and E[Mg(X)]
    fig = make_subplots(rows=1, cols=2)
    
    fig.add_trace(
        go.Scatter(x=ξ_grid, y=np.ones_like(ξ_grid)*QDs_lower[-1]*1.2, name='1.2x min QD', line=dict(color='black',dash='dash')),
        row=1, col=1
    )    
    
    fig.add_trace(
        go.Scatter(x=ξ_grid, y=QDs_lower, name='lower bound', line=dict(color='blue')),
        row=1, col=1
    )

    fig.add_trace(
        go.Scatter(x=ξ_grid, y=QDs_upper, name='upper bound', line=dict(color='purple')),
        row=1, col=1
    )
    ...
    fig.update_layout(height=400, width=1000, title_text="Quadratic divergence (left) and moment bounds (right)", showlegend = False)
...
def box_chart(result_min,result_lower,result_upper,save=False,dpi=1200):
    ...
    if save:
        fig.savefig("box_quadratic_20%.png",dpi=dpi)

# ξs that correspond to 0%-25% higher min QD (g(X)=log Rw)
ξs_lower = np.array([10,
                     0.3267555236852804,
                     0.23304139627543874,
                     0.19162082685130777,
                     0.1672520637512207,
                     0.15048670768737793])
ξs_upper = np.array([10,
                     0.3105597496032715,
                     0.21667560189962387,
                     0.1751637202191887,
                     0.15077531337738037,
